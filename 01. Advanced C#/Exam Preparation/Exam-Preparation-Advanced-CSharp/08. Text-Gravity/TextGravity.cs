...
class TextGravity
{
    static void Main()
    {
        ...
        CreateTextTable(N, M, line, table);
    ...
    private static void PrintTable(int N, int M, char[,] table)
    {
        Console.Write("<table>");
        for (int row = 0; row < N; row++)
        {
            ...
            {
                Console.Write("<td>{0}</td>", SecurityElement.Escape(table[row, col].ToString()));
            }
            Console.Write("</tr>");
        }
        Console.Write("</table>");
    ...
    private static void FallText(int N, int M, char[,] table)
    {
        bool fallen = false;
        do
        {
    ...
    private static void CreateTextTable(int N, int M, string line, char[,] table)
    {
        int index = 0;
        for (int row = 0; row < N; row++)
        {
